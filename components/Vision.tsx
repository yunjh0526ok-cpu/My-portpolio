...
const VisionCard = ({ icon: Icon, title, desc, delay }: { icon: any, title: string, desc: string, delay: number }) => (
  ...
  >
    <div className="relative p-8 h-full bg-slate-900/40 border border-slate-700/50 backdrop-blur-sm rounded-2xl overflow-hidden hover:border-cyber-accent/50 transition-all duration-500 hover:shadow-[0_0_30px_rgba(255,110,30,0.15)]">
      ...
      <div className="relative group/icon inline-flex w-14 h-14 rounded-xl bg-slate-800/80 items-center justify-center mb-6 text-cyber-400 group-hover:text-white group-hover:bg-cyber-500/20 transition-all duration-300 cursor-help">
      ...
      <h3 className="text-2xl font-bold text-white mb-4 group-hover:text-cyber-accent transition-colors">{title}</h3>
      <p className="text-slate-400 leading-relaxed group-hover:text-slate-200 transition-colors break-keep">
        {desc}
      </p>
    </div>
  </motion.div>
...
const BusinessItem = ({ label, sub }: { label: string, sub: string }) => (
    <div className="flex items-center gap-4 p-4 border-b border-slate-800 hover:bg-white/5 transition-colors">
        <div className="w-2 h-2 rounded-full bg-cyber-accent shadow-[0_0_8px_#06b6d4]"></div>
        <div>
            <div className="text-white font-bold text-lg">{label}</div>
            <div className="text-slate-500 text-sm group-hover:text-cyber-400 transition-colors">{sub}</div>
        </div>
    </div>
);
...
const Vision: React.FC = () => {
  return (
    <section id="vision" className="relative z-10 py-24 px-4 w-full max-w-7xl mx-auto scroll-mt-24">
      {/* Section Header */}
      <motion.div 
        initial={{ opacity: 0 }}
      ...
      >
        <span className="text-cyber-accent font-tech tracking-widest text-sm uppercase mb-3 block">Our Vision & Scope</span>
        ...
        <p className="text-slate-300 max-w-3xl mx-auto text-xl md:text-2xl leading-relaxed font-light">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
         ...
         >
             <h3 className="text-3xl font-bold text-white mb-8 border-l-4 border-[#ff6e1e] pl-6">
                Business Scope
             </h3>
             <div className="bg-slate-900/30 backdrop-blur-md rounded-3xl border border-slate-800 overflow-hidden shadow-2xl">
         ...
         <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="relative h-[400px] w-full bg-gradient-to-br from-slate-900 to-[#020205] rounded-3xl border border-slate-800 flex items-center justify-center overflow-hidden group"
         >
            {/* Abstract Tech Graphic */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(6,182,212,0.1),transparent_70%)]" />
            
            {/* Rotating Rings */}
            <div className="absolute w-64 h-64 border border-cyber-500/20 rounded-full animate-[spin_10s_linear_infinite]" />
            <div className="absolute w-48 h-48 border border-[#ff6e1e]/20 rounded-full animate-[spin_15s_linear_infinite_reverse]" />
            
            {/* Center Hub */}
            <div className="relative z-10 text-center">
                <div className="w-20 h-20 mx-auto bg-slate-800 rounded-2xl flex items-center justify-center border border-cyber-accent/30 shadow-[0_0_30px_rgba(6,182,212,0.3)] mb-4 group-hover:scale-110 transition-transform duration-500">
                    <Cpu className="w-10 h-10 text-white" />
                </div>
                <h4 className="text-2xl font-tech text-white mb-2">Ethics Core</h4>
